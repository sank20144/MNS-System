...
VOID DriverUnload(PDRIVER_OBJECT pDriverObject)
{
	...
	UNICODE_STRING MNSSymbolName;
	RtlInitUnicodeString(&MNSSymbolName, L"\\DosDevices\\MNS-Driver"); // Giving the driver a symbol
	if (ObHandle)
	{
	...
	IoDeleteSymbolicLink(&MNSSymbolName);
...
OB_PREOP_CALLBACK_STATUS ThreadPreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
	...
	if ((ULONG)PsGetCurrentProcessId() == UsermodeMNS)
	{
		return OB_PREOP_SUCCESS;
	}
	...
	if (PsGetThreadProcessId(OperationInformation->Object) == UsermodeMNS)
	{
...
OB_PREOP_CALLBACK_STATUS ProcessPreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
	...
	if (UsermodeMNS != 0)
	{
		if (PsGetProcessId((PEPROCESS)OperationInformation->Object) == UsermodeMNS)
		{
...
BOOLEAN UsermodeMNSRecieved = FALSE;
NTSTATUS IoControl(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (ControlCode == IO_SEND_CURRENTPROCESS && UsermodeMNS < 1)
	{
		PKERNEL_READ_REQUEST ReadInput = (PKERNEL_READ_REQUEST)Irp->AssociatedIrp.SystemBuffer;
		UsermodeMNS = ReadInput->UsermodeProgram;

		Status = STATUS_SUCCESS;
		BytesIO = sizeof(KERNEL_READ_REQUEST);
	}
	if ((ULONG)UsermodeMNS == (ULONG)PsGetProcessId(PsGetCurrentProcess()))
	{
		...
		else if (ControlCode == IO_VADPROTECTION)
		{
			...
			if (ReadInput->pid == ProtectedProcess || ReadInput == UsermodeMNS)
			{
				BBHideVAD(ReadInput);
			}
			Status = STATUS_SUCCESS;
	...
	else if ((ULONG)ProtectedProcess == (ULONG)PsGetProcessId(PsGetCurrentProcess()))
	{
		...
		{
			PKERNEL_READ_REQUEST ReadInput = (PKERNEL_READ_REQUEST)Irp->AssociatedIrp.SystemBuffer;
			ReadInput->UsermodeProgram = UsermodeMNS;
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriverObject, PUNICODE_STRING pUniStr)
{
	...
	RtlInitUnicodeString(&DriverName, L"\\Device\\MNS-Driver"); // Giving the driver a name
	RtlInitUnicodeString(&SymbolName, L"\\DosDevices\\MNS-Driver"); // Giving the driver a symbol
